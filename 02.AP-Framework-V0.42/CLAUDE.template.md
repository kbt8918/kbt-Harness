...
## 하네스: AP-Framework 에이전트 팀

**목표:** 17단계 Document Chaining 산출물 생성을 에이전트 팀으로 병렬·자동 조율하고, Gate-Check와 진행상황 추적을 자동화한다.

**트리거:** 산출물 생성, Week N 자동화, 기획/구현/검수 진행, "전체 프로젝트 진행", "다시 실행/재실행/보완" 요청 시 `ap-orchestrator` 스킬을 사용하라. 진행상황 갱신·Gate-Check·전제조건 확인은 `progress-automation` 스킬을 사용하라. 단순 질문은 직접 응답 가능.

**구성:** PM 리더(오케스트레이터) + 기획/구현/QA/진행관리 에이전트. 정의는 `.claude/agents/`, 스킬은 `.claude/skills/`에 있다.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-08 | 초기 하네스 구성 (에이전트 팀 + 진행상황 자동화) | 전체 | 단일 인스턴스 병렬성 한계 + .progress.md 수동 추적 개선 |
